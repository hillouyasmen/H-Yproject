...
router.get("/", async (req, res, next) => {
  try {
    ...
    // هل استُخدمت التجربة المجانية من قبل؟
...
/** POST /api/club  { user_id, bodyshape_id, plan }  (مرة واحدة لاختيار الجسم) */
router.post("/", async (req, res, next) => {
  ...
  try {
    ...
    // معلومات المستخدم + تثبيت bodyshape مرة واحدة
    ...
    // تحقق من حالة الـ trial
    if (plan === "trial") {
      // إن سبق استخدم trial — ارفض
      ...
      // ولو عنده عضوية فعّالة حالياً — ارفض (لا نلغيها)
      ...
      // للخطط المدفوعة: أغلق أي عضوية active حالية قبل الإنشاء
      await conn.query(
