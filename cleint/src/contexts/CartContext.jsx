...
export function CartProvider({ children }) {
  ...
  /** حارس: لا يسمح لغير المسجلين ولا للأدمن */
  const guard = () => {
    if (!user) {
  ...
  /** تنفيذ الطلب – يرفض للأدمن أو لغير المسجل */
  const checkout = async () => {
    ...
    return data; // { order_id, total_amount, ... }
  };

  return (
    <CartContext.Provider
      value={{ items, addToCart, removeFromCart, clearCart, total, checkout }}
    >
      {children}
    </CartContext.Provider>
  );
